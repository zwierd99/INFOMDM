...
def tree_pred(x, tr):
    ...
    :param tr: tree object created with grow tree method
    :return: vector of predicted class labels for x
    """
    y = []
...
def select_feature(current_node, feature_set_indices, minleaf):
    ...
    :param feature_set_indices: indices of the columns which will be compared to each other
    :param minleaf: minimal number of observations remaining in a node after splitting
...
def chance_of(x, total):
    """
    Chance of certain element x in a list
    :param x: wanted value
    :param total: list to search
    :return: occurences of x in total
    """
    count = 0
    for i in total:
        if i == x:
